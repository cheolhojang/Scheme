...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        return eval_all(expressions.second, env) #recursively shortens expressions
    # END PROBLEM 8
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 3
        if symbol in self.bindings: #checks if symbol is defined in the current environment
            return self.bindings[symbol]
        elif self.parent is not None: #checks the parents' environment
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.second is nil:
            boolean = scheme_eval(expressions.first, env, True)
        else:
            boolean = scheme_eval(expressions.first, env)
        if scheme_falsep(boolean):
            return False
